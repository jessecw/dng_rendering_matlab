//
//  dng_rendering_acr3_tone_mapping.c
//  
//
...
static const float default_acr3_tone_curve[] =
{
...
float acr3_tone_curve_evaluate(float x)
{
    float y, fract;
    int index;
    unsigned int kTableSize = sizeof (default_acr3_tone_curve) /sizeof(default_acr3_tone_curve[0]);
    
    y = (float) x * (float) (kTableSize-1);
    
    index = (int)y;
    //y = (y >= 0) ? y : 0;
    //y = (y <= (kTableSize - 2)) ? y : (kTableSize - 2);
    ...
    return (default_acr3_tone_curve[index] * (1.0f - fract) + default_acr3_tone_curve[index + 1] * fract);
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (in_param_category != mxDOUBLE_CLASS){
        ...
        {\
            \
            rr = acr3_tone_curve_evaluate(r);\
            bb = acr3_tone_curve_evaluate(b);\
            \
            gg = bb + ((rr - bb) * (g - b) / (r - b));\
            \
        }
